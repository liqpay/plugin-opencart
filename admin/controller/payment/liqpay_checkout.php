...
class ControllerPaymentLiqPayCheckout extends Controller {
	...
	public function index() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('payment/liqpay_checkout', 'token=' . $this->session->data['token'], 'SSL')
		);

		$data['action'] = $this->url->link('payment/liqpay_checkout', 'token=' . $this->session->data['token'], 'SSL');

		$data['cancel'] = $this->url->link('extension/payment', 'token=' . $this->session->data['token'], 'SSL');

		if (isset($this->request->post['liqpay_checkout_merchant'])) {
			$data['liqpay_checkout_merchant'] = $this->request->post['liqpay_checkout_merchant'];
		} else {
			$data['liqpay_checkout_merchant'] = $this->config->get('liqpay_checkout_merchant');
		}

		if (isset($this->request->post['liqpay_checkout_signature'])) {
			$data['liqpay_checkout_signature'] = $this->request->post['liqpay_checkout_signature'];
		} else {
			$data['liqpay_checkout_signature'] = $this->config->get('liqpay_checkout_signature');
		}
		...
		$this->response->setOutput($this->load->view('payment/liqpay_checkout.tpl', $data));
	}

	protected function validate() {
		if (!$this->user->hasPermission('modify', 'payment/liqpay_checkout')) {
			$this->error['warning'] = $this->language->get('error_permission');
		}

		if (!$this->request->post['liqpay_checkout_merchant']) {
			$this->error['merchant'] = $this->language->get('error_merchant');
		}

		if (!$this->request->post['liqpay_checkout_signature']) {
